...
//********************************************* reading the file and storing the usernames in a char usernames [] array
  ...
  const char* filename_to_be_read = "a.txt";

  char usernames[MAX_FILE_SIZE];
  int usernames_index = 0;
  int usernameCountA = 0;
...
//***************************** sending the number of usernames in a.txt to the Main Server
  sendto(sockfd, &usernameCountA, sizeof(usernameCountA), MSG_CONFIRM, (const struct sockaddr *) &cliaddr, sizeof(cliaddr));
...
//***************************** sending all the usernames in b.txt to the Main Server
  ...
  std::cout<<"Server A finished sending a list of usernames to Main Server."<<std::endl;
  cout << endl; 
  ...
  while(true)
  {
    ...
    int count = 0;
    char existingUsernames[MAX_USERS][MAX_USERNAME_LEN]; // array of valid usernames, that is, usernames entered by the client user that are also in a.txt

    int numberOfExistingUsernames = 0;

//********************************* receiving the number of valid usernames, that is, usernames entered by the client user that are also in a.txt
    recvfrom(sockfd, (int *)&numberOfExistingUsernames, sizeof(int), MSG_WAITALL, (struct sockaddr *) &cliaddr, &len);

//*********************************** if none of the usernames entered by the client user are in a.txt, then the Main server will not send the usernames to Server B
    if(numberOfExistingUsernames > 0)
    {

      // receiving the valid usernames from Main server, only if one or more of the usernames entered by the client user are in a.txt
      ...
      std::cout << "Server A received the usernames from Main Server using UDP over port " << PORT << endl;
      cout << endl;
      ...
      ifstream file_to_be_read("a.txt");
      ...
      // map to store only valid usernames and availabilities (that is, those usernames entered by the client user that are also in a.txt)
